...
Generated from: ('Chassis_State.xml',)
...
MAVLINK_MSG_ID_CHASSIS = 1


class MAVLink_chassis_message(MAVLink_message):
    """
    Test all field types
    """

    id = MAVLINK_MSG_ID_CHASSIS
    msgname = "CHASSIS"
    fieldnames = ["pos_x", "pos_y", "radius", "theta", "state", "flag"]
    ordered_fieldnames = ["radius", "pos_x", "pos_y", "theta", "state", "flag"]
    fieldtypes = ["float", "float", "double", "float", "uint8_t", "uint8_t"]
    fielddisplays_by_name: Dict[str, str] = {}
    fieldenums_by_name: Dict[str, str] = {}
    fieldunits_by_name: Dict[str, str] = {}
    native_format = bytearray(b"<dfffBB")
    orders = [1, 2, 0, 3, 4, 5]
    lengths = [1, 1, 1, 1, 1, 1]
    array_lengths = [0, 0, 0, 0, 0, 0]
    crc_extra = 116
    unpacker = struct.Struct("<dfffBB")
    instance_field = None
    instance_offset = -1

    def __init__(self, pos_x: float, pos_y: float, radius: float, theta: float, state: int, flag: int):
        MAVLink_message.__init__(self, MAVLink_chassis_message.id, MAVLink_chassis_message.msgname)
        self._fieldnames = MAVLink_chassis_message.fieldnames
        self._instance_field = MAVLink_chassis_message.instance_field
        self._instance_offset = MAVLink_chassis_message.instance_offset
        self.pos_x = pos_x
        self.pos_y = pos_y
        self.radius = radius
        self.theta = theta
    ...
    def pack(self, mav: "MAVLink", force_mavlink1: bool = False) -> bytes:
        return self._pack(mav, self.crc_extra, self.unpacker.pack(self.radius, self.pos_x, self.pos_y, self.theta, self.state, self.flag), force_mavlink1=force_mavlink1)


# Define name on the class for backwards compatibility (it is now msgname).
# Done with setattr to hide the class variable from mypy.
setattr(MAVLink_chassis_message, "name", mavlink_msg_deprecated_name_property())


mavlink_map: Dict[int, Type[MAVLink_message]] = {
    MAVLINK_MSG_ID_CHASSIS: MAVLink_chassis_message,
}
...
class MAVLink(object):
    ...
    def chassis_encode(self, pos_x: float, pos_y: float, radius: float, theta: float, state: int, flag: int) -> MAVLink_chassis_message:
        """
        Test all field types

        pos_x                     : x Position (type:float)
        pos_y                     : x Position (type:float)
        radius                    : radius of weight (type:double)
        theta                     : Heading angle (type:float)
        state                     : state machine (type:uint8_t)
        flag                      : left or right (type:uint8_t)

        """
        return MAVLink_chassis_message(pos_x, pos_y, radius, theta, state, flag)

    def chassis_send(self, pos_x: float, pos_y: float, radius: float, theta: float, state: int, flag: int, force_mavlink1: bool = False) -> None:
        """
        Test all field types
        ...
        """
        self.send(self.chassis_encode(pos_x, pos_y, radius, theta, state, flag), force_mavlink1=force_mavlink1)
